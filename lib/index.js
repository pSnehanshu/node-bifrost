...
class Bifrost {
     ...
     */
    addParentScope(scope, parentScope) {
        // We shouldn't be able to add itself a parent scope of itself.
        if (scope == parentScope) {
            throw new Error(`Trying to make '${parentScope}' as a parent scope of itself.`);
        }

        // Check if the parentScope is already child.
        // We won't want to make a child as a parent.
        // This will cause infinite recursions.
        var childScopes = this.getChildScopes(scope);
        if (childScopes.includes(parentScope)) {
            throw new Error(`Trying to make '${parentScope}' a parent scope of '${scope}' which is already a child scope.`);
        }

        if (!this.scopeMap[parentScope]) this.scopeMap[parentScope] = [];
        var _scope = this.scopeMap[parentScope];

        if (!_scope.includes(scope)) {
            _scope.push(scope);
        }
    }
     ...
     */
    getChildScopes(scope, recursive = true) {
        var children = [];
        var directChildren = this.scopeMap[scope] || [];
        for (var i = 0; i < directChildren.length; i++) {
            children.push(directChildren[i]);

            // Fetch parentScopes of cScope as well
            if (recursive) {
                let cScopes = this.getChildScopes(directChildren[i]);
     ...
     */
    getParentScopes(scope, recursive = true) {
        var parentScopes = [];
        for (var pScope in this.scopeMap) {
            if (this.scopeMap.hasOwnProperty(pScope)) {
                if (this.scopeMap[pScope].includes(scope)) {
                    parentScopes.push(pScope);

                    // Fetch parentScopes of pScope as well
                    if (recursive) {
                        let ppScopes = this.getParentScopes(pScope);
                        parentScopes = arrayUnique(parentScopes.concat(ppScopes));
                    }
                }
            }
        }
