class Bifrost {
    ...
    assign(user, role, scope = '__global') {
        ...
        if (!this.userRoleScope[user]) this.userRoleScope[user] = {};
        var _user = this.userRoleScope[user];
     ...
     * @returns {Boolean}
     */
    allowed(user, resource, action, scope = '__global') {
        /*
        ...
        */
        try {
            // #1
            var roles = this.userRoleScope[user];
