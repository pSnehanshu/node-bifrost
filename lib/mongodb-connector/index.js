...
class MongoDbConnector {
    ...
    /**
     * Removes roles and scopes from a user
     * @param {String} user 
     * @param {String} role
     * @param {String} scope
     * @returns Promise which resolves to the updated set of roles object
     */
    async removeUserRoles(user, role, scope) {
        this.checkReady();

        var roles = await this.getRolesWithScopes(user);

        // No roles assigned, nothing to do
        if (Array.isArray(roles[role])) {
            // Now do the actual removal
            // TODO: Remove duplicate scopes
            roles[role] = arrayUnique(roles[role]);

            // Check if the given scope is a part of the role and remove it
            if (roles[role].includes(scope)) {
                var index = roles[role].indexOf(scope);
                roles[role].splice(index, 1);

                // If no scope remains altogether, then remove the role entirely
    ...
    /**
     * Adds a parent scope to a child scope
     * @param {String} child The child Scope
     * @param {String} parent The parent scope
     */
    addParentScope(child, parent) {
        this.checkReady();

        return new Promise((resolve, reject) => {
            this._scopesCollection.insertOne({ parent, child })
